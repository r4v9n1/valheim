...
## 0.3.16 - Shared physical ocean core

### Physics
- Re-enabled the CPU ripple field as a physics-only simulation. The rendered Valheim mesh remains geometrically stable, preserving the anti-jelly rule.
- Gameplay water height now samples base sea level + deterministic wind waves + the live ripple field instead of returning a flat sea level.
- Added physical surface-normal and water-velocity sampling so buoyant bodies can react to wave slope and moving water.
- Player, creature, floating-object, and ship disturbances now inject energy into the real ripple field while still emitting foam/splash visuals.

### Boats
- Added six-point hull buoyancy sampling across bow, midship, and stern. Each probe samples PhysicalWater wave height, surface normal, and local water velocity and applies bounded force-at-position buoyancy/drag.
- Buoyancy simulation remains network-owner gated.
- Kept Valheim ship control/propulsion code active for compatibility, but the water surface consumed by the ship is now PhysicalWater and the additional flotation comes from the PhysicalWater multipoint field rather than a single lift point.
- Moving stern probes inject real wake disturbance into the shared ripple field.

### Rendering / shoreline
- Physical wave/ripple state is shared with shader globals and interaction layers while mesh vertices remain static.
- Disabled local-grid edge-only ocean connectivity as the default replacement mode so valid below-sea-level water cannot disappear simply because the current follow-grid boundary is land. Terrain masking still prevents water triangles on dry ground.
- Retained the D3D11 UnityPhysicalOcean shader/material pipeline and procedural normal/foam/noise textures.

### Defaults / workflow
- `WindWaveAmplitude` now defaults to `1.00`.
- Ripple speed/damping now use `8.5` / `0.965`.
- `MaxSimulatedDisplacement` is raised to `0.55 m` for visible physical interaction without the former mesh-sheet instability.
- `RequireOceanConnection` now defaults to `false` for the moving local grid.
- `PhysicalWaterInteractionEnabled` defaults to `true`; dry-baseline mode defaults to `false`.
- Added 0.3.16 build/install scripts and a BOM-free BAT launcher. DLL build must report assembly version `0.3.16.0`.
