...
$VersionedChangelogPath = Join-Path $ProjectRoot "CHANGELOG-0.4.5.md"
$CanonicalChangelogPath = Join-Path $ProjectRoot "CHANGELOG.md"
if (!(Test-Path -LiteralPath $VersionedChangelogPath -PathType Leaf)) {
    throw "Missing mandatory 0.4.5 changelog: $VersionedChangelogPath"
}
Copy-Item -LiteralPath $VersionedChangelogPath -Destination $CanonicalChangelogPath -Force
Write-Host "Applied CHANGELOG 0.4.5 to canonical project changelog." -ForegroundColor DarkGray
...
$FixedShaderPath = Join-Path $UnityPackage "Runtime\Shaders\PhysicalOceanSurface-0.4.5.shader.fixed"
...
if (!(Test-Path -LiteralPath $FixedShaderPath -PathType Leaf)) {
    throw "Missing mandatory 0.4.5 shader: $FixedShaderPath"
}
Copy-Item -LiteralPath $FixedShaderPath -Destination $CanonicalShaderPath -Force
Write-Host "Applied PhysicalOceanSurface 0.4.5 concentric LOD spectral ocean shader to canonical G: source." -ForegroundColor DarkGray
...
$FixedBundleBuilderPath = Join-Path $ProjectRoot "BuildPhysicalWaterBundle-0.4.5.cs.fixed"
...
if (!(Test-Path -LiteralPath $FixedBundleBuilderPath -PathType Leaf)) {
    throw "Missing mandatory 0.4.5 AssetBundle builder: $FixedBundleBuilderPath"
}
...
& (Join-Path $ProjectRoot "build-0.4.5.ps1") -ValheimDir $ValheimDir -Configuration $Configuration
...
Write-Host ""
Write-Host "PhysicalWater 0.4.5 milestone: bathymetry-aware shallow water, static obstacle interaction, continuous GPU surf/rock foam, smoothed local ripples, and origin-preserved simulation state."
