...
$VersionedChangelogPath = Join-Path $ProjectRoot "CHANGELOG-0.3.15.md"
...
if (Test-Path -LiteralPath $VersionedChangelogPath -PathType Leaf) {
    ...
    Write-Host "Applied CHANGELOG 0.3.15 to canonical project changelog." -ForegroundColor DarkGray
}
...
$FixedShaderPath = Join-Path $UnityPackage "Runtime\Shaders\PhysicalOceanSurface-0.3.15.shader.fixed"
...
if (Test-Path -LiteralPath $FixedShaderPath -PathType Leaf) {
    ...
    Write-Host "Applied PhysicalOceanSurface 0.3.15 animated D3D11 surface shader fix to canonical G: source." -ForegroundColor DarkGray
}
...
$FixedBundleBuilderPath = Join-Path $ProjectRoot "BuildPhysicalWaterBundle-0.3.15.cs.fixed"
...
& (Join-Path $ProjectRoot "build-0.3.15.ps1") -ValheimDir $ValheimDir -Configuration $Configuration
...
Write-Host ""
Write-Host "PhysicalWater 0.3.15 milestone: native ship rigidbody ownership, launch-force removal, shoreline slab removal, animated optical waves, stronger contact foam/splashes."
