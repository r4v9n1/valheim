...
$PatchedPatchesSource = Join-Path $ProjectRoot "PhysicalWaterPatches-0.3.17.cs.fixed"
$ProjectPatchesPath = Join-Path $ProjectRoot "src\PhysicalWaterPatches.cs"
$PatchedSystemSource = Join-Path $ProjectRoot "PhysicalWaterSystem-0.3.17.cs.fixed"
$ProjectSystemPath = Join-Path $ProjectRoot "src\PhysicalWaterSystem.cs"
$PatchedPluginSource = Join-Path $ProjectRoot "PhysicalWaterPlugin-0.3.17.cs.fixed"
...
if (Test-Path -LiteralPath $PatchedPatchesSource -PathType Leaf) {
    ...
    Write-Host "Applied PhysicalWaterPatches 0.3.17 authoritative no-vanilla water/ship solver." -ForegroundColor Cyan
}
...
if (Test-Path -LiteralPath $PatchedSystemSource -PathType Leaf) {
    ...
    Write-Host "Applied PhysicalWaterSystem 0.3.17 shared GPU/CPU wave + live ripple render bridge." -ForegroundColor Cyan
}
...
if (Test-Path -LiteralPath $PatchedPluginSource -PathType Leaf) {
    ...
    Write-Host "Applied PhysicalWaterPlugin 0.3.17 version bump." -ForegroundColor Cyan
}
...
if ($dllVersion -ne "0.3.17.0") {
    throw "Fresh DLL version is $dllVersion, expected 0.3.17.0."
}
