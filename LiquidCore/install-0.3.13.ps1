...
$FixedShaderPath = Join-Path $UnityPackage "Runtime\Shaders\PhysicalOceanSurface.shader.fixed"
...
if (Test-Path -LiteralPath $FixedShaderPath -PathType Leaf) {
    ...
    Write-Host "Applied corrected PhysicalOceanSurface.shader to canonical G: source." -ForegroundColor DarkGray
}
...
$FixedBundleBuilderPath = Join-Path $ProjectRoot "BuildPhysicalWaterBundle-0.3.13.cs.fixed"
...
& (Join-Path $ProjectRoot "build-0.3.13.ps1") -ValheimDir $ValheimDir -Configuration $Configuration
...
Write-Host ""
Write-Host "PhysicalWater 0.3.13 milestone: forward renderer-readiness, corrected ship buoyancy, deterministic force-rebuilt Unity bundle, and versioned patch workflow."
