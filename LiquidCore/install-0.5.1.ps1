...
$VersionedChangelogPath = Join-Path $ProjectRoot "CHANGELOG-0.5.1.md"
$CanonicalChangelogPath = Join-Path $ProjectRoot "CHANGELOG.md"
if (!(Test-Path -LiteralPath $VersionedChangelogPath -PathType Leaf)) {
    throw "Missing mandatory 0.5.1 changelog: $VersionedChangelogPath"
}
Copy-Item -LiteralPath $VersionedChangelogPath -Destination $CanonicalChangelogPath -Force
Write-Host "Applied CHANGELOG 0.5.1 to canonical project changelog." -ForegroundColor DarkGray
...
$FixedShaderPath = Join-Path $UnityPackage "Runtime\Shaders\PhysicalOceanSurface-0.5.1.shader.fixed"
...
if (!(Test-Path -LiteralPath $FixedShaderPath -PathType Leaf)) {
    throw "Missing mandatory 0.5.1 shader: $FixedShaderPath"
}
Copy-Item -LiteralPath $FixedShaderPath -Destination $CanonicalShaderPath -Force
Write-Host "Applied PhysicalOceanSurface 0.5.1 concentric LOD spectral ocean shader to canonical G: source." -ForegroundColor DarkGray
...
$FixedBundleBuilderPath = Join-Path $ProjectRoot "BuildPhysicalWaterBundle-0.5.1.cs.fixed"
...
if (!(Test-Path -LiteralPath $FixedBundleBuilderPath -PathType Leaf)) {
    throw "Missing mandatory 0.5.1 AssetBundle builder: $FixedBundleBuilderPath"
}
...
& (Join-Path $ProjectRoot "build-0.5.1.ps1") -ValheimDir $ValheimDir -Configuration $Configuration
...
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Visuals" -Key "ShorelineEffectsEnabled" -Value "false"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Visuals" -Key "ShorelineEffectsInterval" -Value "0.32"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Visuals" -Key "ShorelineEffectsBudget" -Value "0"
...
Write-Host ""
Write-Host "PhysicalWater 0.5.1 milestone: audited conservative shallow-water hydrodynamics, ocean-domain boundary seeding, stateful active-tile wet/dry flow, static wall face barriers, and no sea-level blanket fill."
