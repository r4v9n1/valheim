...
function Ensure-BepInExRefs {
    ...
    if (!(Test-Path -LiteralPath $bepInExDll) -or !(Test-Path -LiteralPath $harmonyDll)) {
        ...
        Expand-Zip -ZipPath $packagePath -Destination $extractPath
...
if ($sdkList) {
    dotnet build (Join-Path $ProjectRoot "Terramizer.csproj") `
        -c $Configuration `
    ...
    $outPath = Join-Path $distDir "Terramizer.dll"
...
Write-Host "Built: $(Join-Path $distDir 'Terramizer.dll')"
