...
if ($sdkList) {
    dotnet build (Join-Path $ProjectRoot "InventoryLink.csproj") `
        -c $Configuration `
    ...
    $refs = @(
        ...
        (Join-Path $GameManagedDir "SoftReferenceableAssets.dll"),
    ...
    $outPath = Join-Path $distDir "InventoryLink.dll"
...
Write-Host "Built: $(Join-Path $distDir 'InventoryLink.dll')"
