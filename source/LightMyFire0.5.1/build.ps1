...
$DllPath = Join-Path $DistDir "LightMyFire.dll"
...
if (!(Test-Path -LiteralPath $AssetBundlePath -PathType Leaf)) {
    throw "Missing verified production AssetBundle: $AssetBundlePath"
}
...
if ($assetHash -ne $ExpectedAssetSha256) {
    throw "AssetBundle mismatch. Expected $ExpectedAssetSha256 but found $assetHash."
}

New-Item -ItemType Directory -Force -Path $DistDir | Out-Null

# Force a fresh DLL. Never let an old dist DLL survive into a release package.
if (Test-Path -LiteralPath $DllPath) { Remove-Item -LiteralPath $DllPath -Force }

dotnet build (Join-Path $ProjectRoot "LightMyFire.csproj") `
    -c $Configuration `
    --no-incremental `
...
if ($dllVersion -ne "0.5.1.0") { throw "Fresh DLL version is $dllVersion, expected 0.5.1.0." }

Write-Host "Built fresh LightMyFire.dll" -ForegroundColor Green
