...
Write-Host "[2/3] Building fresh LightMyFire 0.5.1 DLL..." -ForegroundColor Cyan
& (Join-Path $Root "build.ps1") -ValheimDir $ValheimDir
if ($LASTEXITCODE -ne 0) { throw "DLL build failed." }

Write-Host "[3/3] Packaging LightMyFire_Coal_Resin 0.5.1..." -ForegroundColor Cyan
& (Join-Path $Root "package.ps1") -NoBuild
if ($LASTEXITCODE -ne 0) { throw "Packaging failed." }

Write-Host "Done: artifacts\R4V9N1-LightMyFire_Coal_Resin-0.5.1.zip" -ForegroundColor Green
