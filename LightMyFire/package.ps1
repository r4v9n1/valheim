...
$ReleaseDir = "G:\My Drive\build\Valheim\releases\LightMyFire"
$Owner = "R4V9N1"
...
if ($pluginSource -notmatch 'PluginVersion = "1\.0\.1"') { throw "Plugin source version is not 1.0.1." }
if ($manifest.description -notmatch "I use AI" -or $manifest.description -notmatch "manually test" -or $manifest.description -notmatch "remain my own") {
    throw "Manifest description must include the AI-assisted development disclosure without human-directed wording."
}
$readmeText = Get-Content -LiteralPath (Join-Path $ThunderstoreDir "README.md") -Raw
