...
namespace LightMyFire
{
    ...
    internal static class LightMyFireFeeder
    {
        ...
        private static int TryApplyFuelToFireplace(ZDOID fireplaceId, int grantedAmount)
        {
            ...
            // Do not call Fireplace.AddFuel here. In the current Valheim build that method routes
            // through RPC_AddFuelAmount, which always creates the source's fuel-added EffectList
            // (including its sound). Automatic barrel refills can happen for many sources at once,
            // so update the authoritative fuel value directly and let the source's normal state
            // update/replication refresh its visuals without spawning refill feedback.
            zdo.Set(ZDOVars.s_fuel, Mathf.Clamp(currentFuel + applied, 0f, fireplace.m_maxFuel));
