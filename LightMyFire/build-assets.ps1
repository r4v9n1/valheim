...
$LocalProjectRoot = Join-Path $env:LOCALAPPDATA "R4V9N1\LightMyFire"
...
$OutputDir = if ($ReplaceProductionAsset) { Join-Path $ProjectRoot "assets" } else { Join-Path $LocalProjectRoot "generated-assets" }
$PreviewPath = Join-Path $LocalProjectRoot "LightMyFire-model-preview.png"
$LogPath = Join-Path $LocalProjectRoot "LightMyFire-unity-build.log"

if (!$ReplaceProductionAsset) {
    Write-Host "SAFE MODE: generated AssetBundle goes to local disposable storage and will NOT replace the verified 0.5.1 production bundle." -ForegroundColor Yellow
