...
if ($pluginSource -notmatch 'PluginVersion = "0\.5\.4"') { throw "Plugin source version is not 0.5.4." }
...
Write-Host "[2/3] Building fresh LightMyFire 0.5.4 DLL..." -ForegroundColor Cyan
& (Join-Path $Root "build.ps1") -ValheimDir $ValheimDir
if ($LASTEXITCODE -ne 0) { throw "DLL build failed." }

Write-Host "[3/3] Packaging LightMyFire_Coal_Resin 0.5.4..." -ForegroundColor Cyan
& (Join-Path $Root "package.ps1") -NoBuild
if ($LASTEXITCODE -ne 0) { throw "Packaging failed." }

Write-Host "Done: G:\My Drive\build\Valheim\releases\LightMyFire\R4V9N1-LightMyFire_Coal_Resin-0.5.4.zip" -ForegroundColor Green
