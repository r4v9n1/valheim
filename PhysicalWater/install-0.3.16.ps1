...
$VersionedChangelogPath = Join-Path $ProjectRoot "CHANGELOG-0.3.16.md"
...
if (Test-Path -LiteralPath $VersionedChangelogPath -PathType Leaf) {
    ...
    Write-Host "Applied CHANGELOG 0.3.16 to canonical project changelog." -ForegroundColor DarkGray
}
...
$FixedShaderPath = Join-Path $UnityPackage "Runtime\Shaders\PhysicalOceanSurface-0.3.16.shader.fixed"
...
if (Test-Path -LiteralPath $FixedShaderPath -PathType Leaf) {
    ...
    Write-Host "Applied PhysicalOceanSurface 0.3.16 animated D3D11 surface shader fix to canonical G: source." -ForegroundColor DarkGray
}
...
$FixedBundleBuilderPath = Join-Path $ProjectRoot "BuildPhysicalWaterBundle-0.3.16.cs.fixed"
...
& (Join-Path $ProjectRoot "build-0.3.16.ps1") -ValheimDir $ValheimDir -Configuration $Configuration
...
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Simulation" -Key "WaveSpeed" -Value "8.5"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Simulation" -Key "Damping" -Value "0.965"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Simulation" -Key "WindWaveAmplitude" -Value "1.00"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Simulation" -Key "WindWaveLength" -Value "38"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Simulation" -Key "MaxSimulatedDisplacement" -Value "0.55"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Simulation" -Key "ObjectDisturbanceScale" -Value "0.28"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Simulation" -Key "CharacterDisturbanceScale" -Value "0.32"
...
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "ShipPhysics" -Key "ShipWakeStrength" -Value "0.06"
...
Write-Host ""
Write-Host "PhysicalWater 0.3.16 milestone: shared physical wave+ripple field, animated gameplay surface, six-probe ship buoyancy, physical wakes/interactions, stable non-jelly render mesh."
