...
$VersionedChangelogPath = Join-Path $ProjectRoot "CHANGELOG-0.4.1.md"
$CanonicalChangelogPath = Join-Path $ProjectRoot "CHANGELOG.md"
if (!(Test-Path -LiteralPath $VersionedChangelogPath -PathType Leaf)) {
    throw "Missing mandatory 0.4.1 changelog: $VersionedChangelogPath"
}
Copy-Item -LiteralPath $VersionedChangelogPath -Destination $CanonicalChangelogPath -Force
Write-Host "Applied CHANGELOG 0.4.1 to canonical project changelog." -ForegroundColor DarkGray
...
$FixedShaderPath = Join-Path $UnityPackage "Runtime\Shaders\PhysicalOceanSurface-0.4.1.shader.fixed"
...
if (!(Test-Path -LiteralPath $FixedShaderPath -PathType Leaf)) {
    throw "Missing mandatory 0.4.1 shader: $FixedShaderPath"
}
Copy-Item -LiteralPath $FixedShaderPath -Destination $CanonicalShaderPath -Force
Write-Host "Applied PhysicalOceanSurface 0.4.1 concentric LOD spectral ocean shader to canonical G: source." -ForegroundColor DarkGray
...
$FixedBundleBuilderPath = Join-Path $ProjectRoot "BuildPhysicalWaterBundle-0.4.1.cs.fixed"
...
if (!(Test-Path -LiteralPath $FixedBundleBuilderPath -PathType Leaf)) {
    throw "Missing mandatory 0.4.1 AssetBundle builder: $FixedBundleBuilderPath"
}
...
& (Join-Path $ProjectRoot "build-0.4.1.ps1") -ValheimDir $ValheimDir -Configuration $Configuration
...
Write-Host ""
Write-Host "PhysicalWater 0.4.1 milestone: Ocean Core Rewrite - concentric camera LOD ocean, deep-water dispersion spectrum shared by CPU/GPU, local interaction field, hydrostatic 3x3 ship buoyancy, no vanilla water."
