...
$PatchedPatchesSource = Join-Path $ProjectRoot "PhysicalWaterPatches-0.3.13.cs.fixed"
$ProjectPatchesPath = Join-Path $ProjectRoot "src\PhysicalWaterPatches.cs"
$PatchedSystemSource = Join-Path $ProjectRoot "PhysicalWaterSystem-0.3.13.cs.fixed"
$ProjectSystemPath = Join-Path $ProjectRoot "src\PhysicalWaterSystem.cs"
$PatchedPluginSource = Join-Path $ProjectRoot "PhysicalWaterPlugin-0.3.13.cs.fixed"
...
if (Test-Path -LiteralPath $PatchedPatchesSource -PathType Leaf) {
    ...
    Write-Host "Applied PhysicalWaterPatches 0.3.13 multiplayer ownership fix." -ForegroundColor Cyan
}
...
if (Test-Path -LiteralPath $PatchedSystemSource -PathType Leaf) {
    ...
    Write-Host "Applied PhysicalWaterSystem 0.3.13 forward visual-readiness fix." -ForegroundColor Cyan
}
...
if (Test-Path -LiteralPath $PatchedPluginSource -PathType Leaf) {
    ...
    Write-Host "Applied PhysicalWaterPlugin 0.3.13 version bump." -ForegroundColor Cyan
}
...
if ($dllVersion -ne "0.3.13.0") {
    throw "Fresh DLL version is $dllVersion, expected 0.3.13.0."
}
