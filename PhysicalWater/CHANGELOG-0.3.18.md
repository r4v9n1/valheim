...
## 0.3.18 - AudioModule compatibility / authoritative-water build fix

- Fixed the 0.3.17 CS1705 build failure caused by Valheim's `UnityEngine.AudioModule` targeting netstandard 2.1 while PhysicalWater targets net472/netstandard 2.0.
- Removed the compile-time `UnityEngine.AudioModule` reference from the PhysicalWater project.
- Reworked PhysicalWater ambient-ocean and splash audio through a runtime reflection bridge, preserving custom PhysicalWater audio without changing the mod target framework.
- Vanilla water audio suppression is also reflection-based; no Valheim water audio is adopted as a fallback.
- Preserves the 0.3.17 hard cutover: no vanilla water renderer, water physics, ship buoyancy, floater simulation, or water query authority while PhysicalWater is enabled.
- Preserves shared CPU/GPU physical waves, live ripple texture, multipoint ship physics, PhysicalWater-only material/shader rendering, interaction effects, and emergency PhysicalWater-owned rendering.
- Build script now promotes a versioned `.csproj` snapshot before compilation so dependency changes are deterministic and versioned with each patch.
