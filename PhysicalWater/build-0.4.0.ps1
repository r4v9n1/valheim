...
$PatchedPatchesSource = Join-Path $ProjectRoot "PhysicalWaterPatches-0.4.0.cs.fixed"
$ProjectPatchesPath = Join-Path $ProjectRoot "src\PhysicalWaterPatches.cs"
$PatchedSystemSource = Join-Path $ProjectRoot "PhysicalWaterSystem-0.4.0.cs.fixed"
$ProjectSystemPath = Join-Path $ProjectRoot "src\PhysicalWaterSystem.cs"
$PatchedPluginSource = Join-Path $ProjectRoot "PhysicalWaterPlugin-0.4.0.cs.fixed"
$ProjectPluginPath = Join-Path $ProjectRoot "src\PhysicalWaterPlugin.cs"
$PatchedProjectFile = Join-Path $ProjectRoot "PhysicalWater-0.4.0.csproj.fixed"
$ProjectFile = Join-Path $ProjectRoot "PhysicalWater.csproj"
$PatchedLodSource = Join-Path $ProjectRoot "PhysicalWaterLodOcean-0.4.0.cs.fixed"
...
if (Test-Path -LiteralPath $PatchedPatchesSource -PathType Leaf) {
    ...
    Write-Host "Applied PhysicalWaterPatches 0.4.0 hydrostatic boat + no-vanilla solver." -ForegroundColor Cyan
}
...
if (Test-Path -LiteralPath $PatchedSystemSource -PathType Leaf) {
    ...
    Write-Host "Applied PhysicalWaterSystem 0.4.0 Ocean Core Rewrite." -ForegroundColor Cyan
}
...
if (Test-Path -LiteralPath $PatchedPluginSource -PathType Leaf) {
    ...
    Write-Host "Applied PhysicalWaterPlugin 0.4.0 version bump." -ForegroundColor Cyan
}


if (Test-Path -LiteralPath $PatchedLodSource -PathType Leaf) {
    Copy-Item -LiteralPath $PatchedLodSource -Destination $ProjectLodPath -Force
    Write-Host "Applied PhysicalWaterLodOcean 0.4.0 concentric ocean renderer." -ForegroundColor Cyan
}
...
if (Test-Path -LiteralPath $PatchedProjectFile -PathType Leaf) {
    ...
    Write-Host "Applied PhysicalWater 0.4.0 net472 project reference fix (AudioModule reflection bridge)." -ForegroundColor Cyan
}
...
if ($dllVersion -ne "0.4.0.0") {
    throw "Fresh DLL version is $dllVersion, expected 0.4.0.0."
}
