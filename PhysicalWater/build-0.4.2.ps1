...
$PatchedPatchesSource = Join-Path $ProjectRoot "PhysicalWaterPatches-0.4.2.cs.fixed"
$ProjectPatchesPath = Join-Path $ProjectRoot "src\PhysicalWaterPatches.cs"
$PatchedSystemSource = Join-Path $ProjectRoot "PhysicalWaterSystem-0.4.2.cs.fixed"
$ProjectSystemPath = Join-Path $ProjectRoot "src\PhysicalWaterSystem.cs"
$PatchedPluginSource = Join-Path $ProjectRoot "PhysicalWaterPlugin-0.4.2.cs.fixed"
$ProjectPluginPath = Join-Path $ProjectRoot "src\PhysicalWaterPlugin.cs"
$PatchedProjectFile = Join-Path $ProjectRoot "PhysicalWater-0.4.2.csproj.fixed"
$ProjectFile = Join-Path $ProjectRoot "PhysicalWater.csproj"
$PatchedLodSource = Join-Path $ProjectRoot "PhysicalWaterLodOcean-0.4.2.cs.fixed"
...
foreach ($requiredPatch in @(
    ...
    if (!(Test-Path -LiteralPath $requiredPatch -PathType Leaf)) {
        throw "Missing mandatory 0.4.2 build input: $requiredPatch"
    }
}

Copy-Item -LiteralPath $PatchedPatchesSource -Destination $ProjectPatchesPath -Force
Write-Host "Applied PhysicalWaterPatches 0.4.2 visual-fidelity carry-forward no-vanilla solver." -ForegroundColor Cyan
Copy-Item -LiteralPath $PatchedSystemSource -Destination $ProjectSystemPath -Force
Write-Host "Applied PhysicalWaterSystem 0.4.2 Visual Fidelity." -ForegroundColor Cyan
Copy-Item -LiteralPath $PatchedPluginSource -Destination $ProjectPluginPath -Force
Write-Host "Applied PhysicalWaterPlugin 0.4.2 version bump." -ForegroundColor Cyan
Copy-Item -LiteralPath $PatchedLodSource -Destination $ProjectLodPath -Force
Write-Host "Applied PhysicalWaterLodOcean 0.4.2 concentric ocean renderer (0.4.1 foundation preserved)." -ForegroundColor Cyan
Copy-Item -LiteralPath $PatchedProjectFile -Destination $ProjectFile -Force
Write-Host "Applied PhysicalWater 0.4.2 net472 project reference fix (AudioModule reflection bridge)." -ForegroundColor Cyan
...
if ($dllVersion -ne "0.4.2.0") {
    throw "Fresh DLL version is $dllVersion, expected 0.4.2.0."
}
