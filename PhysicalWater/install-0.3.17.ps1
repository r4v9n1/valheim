...
$VersionedChangelogPath = Join-Path $ProjectRoot "CHANGELOG-0.3.17.md"
...
if (Test-Path -LiteralPath $VersionedChangelogPath -PathType Leaf) {
    ...
    Write-Host "Applied CHANGELOG 0.3.17 to canonical project changelog." -ForegroundColor DarkGray
}
...
$FixedShaderPath = Join-Path $UnityPackage "Runtime\Shaders\PhysicalOceanSurface-0.3.17.shader.fixed"
...
if (Test-Path -LiteralPath $FixedShaderPath -PathType Leaf) {
    ...
    Write-Host "Applied PhysicalOceanSurface 0.3.17 authoritative GPU physical-wave + live-ripple shader to canonical G: source." -ForegroundColor DarkGray
}
...
$FixedBundleBuilderPath = Join-Path $ProjectRoot "BuildPhysicalWaterBundle-0.3.17.cs.fixed"
...
& (Join-Path $ProjectRoot "build-0.3.17.ps1") -ValheimDir $ValheimDir -Configuration $Configuration
...
Write-Host ""
Write-Host "PhysicalWater 0.3.17 milestone: hard no-vanilla cutover, exclusive PhysicalWater buoyancy/propulsion, low-pass character waterline, GPU physical waves, live ripple texture, own audio/effects, and emergency self-owned visibility material."
