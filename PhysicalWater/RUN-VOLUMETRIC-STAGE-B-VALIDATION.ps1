...
$LocalUnityLog = Join-Path $LocalUnityProject "VolumetricStageBValidationUnity.log"
$LocalReport = Join-Path $LocalUnityProject "VolumetricStageBValidation.log"
$DriveUnityLog = Join-Path $UnityProject "VolumetricStageBValidationUnity.log"
$DriveReport = Join-Path $UnityProject "VolumetricStageBValidation.log"

Write-Host ""
Write-Host " PhysicalWater 0.6.0-devB4 - VOLUMETRIC STAGE-B FLIP VALIDATION" -ForegroundColor Cyan
Write-Host ""
...
$required = @(
    ...
    (Join-Path $UnityProject "Assets\Editor\RunVolumetricStageBValidation.cs")
)
foreach ($path in $required) {
    if (!(Test-Path -LiteralPath $path -PathType Leaf)) { throw "Missing mandatory Stage-B validation input: $path" }
}

Write-Host "Preparing local Unity Stage-B validation workspace..." -ForegroundColor Cyan
...
Write-Host "Running Unity FLIP/free-surface validation..." -ForegroundColor Cyan
$psi = New-Object System.Diagnostics.ProcessStartInfo
$psi.FileName = $UnityExe
$psi.Arguments = "-batchmode -force-d3d11 -quit -projectPath `"$LocalUnityProject`" -executeMethod RunVolumetricStageBValidation.Run -logFile `"$LocalUnityLog`""
...
if ($exitCode -ne 0) {
    ...
    throw "Volumetric Stage-B Unity validation failed with exit code $exitCode.$hint"
}
...
if ($reportText -notmatch "RESULT:\s+PASS") { throw "Volumetric Stage-B report did not contain PASS. See $DriveReport" }
Write-Host "Volumetric Stage-B validation PASSED." -ForegroundColor Green
