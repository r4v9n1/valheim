...
$VersionedChangelogPath = Join-Path $ProjectRoot "CHANGELOG-0.3.14.md"
...
if (Test-Path -LiteralPath $VersionedChangelogPath -PathType Leaf) {
    ...
    Write-Host "Applied CHANGELOG 0.3.14 to canonical project changelog." -ForegroundColor DarkGray
}
...
$FixedShaderPath = Join-Path $UnityPackage "Runtime\Shaders\PhysicalOceanSurface-0.3.14.shader.fixed"
...
if (Test-Path -LiteralPath $FixedShaderPath -PathType Leaf) {
    ...
    Write-Host "Applied PhysicalOceanSurface 0.3.14 D3D11 shader fix to canonical G: source." -ForegroundColor DarkGray
}
...
$FixedBundleBuilderPath = Join-Path $ProjectRoot "BuildPhysicalWaterBundle-0.3.14.cs.fixed"
...
& (Join-Path $ProjectRoot "build-0.3.14.ps1") -ValheimDir $ValheimDir -Configuration $Configuration
...
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "WaterShape" -Key "FollowRadius" -Value "384"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "WaterShape" -Key "OriginSnapMeters" -Value "96"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "WaterShape" -Key "TerrainMaskEnabled" -Value "true"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "WaterShape" -Key "RequireOceanConnection" -Value "true"
...
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "WaterShape" -Key "FarOceanEnabled" -Value "true"
...
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Visuals" -Key "ShorelineEffectsInterval" -Value "0.22"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Visuals" -Key "ShorelineEffectsBudget" -Value "160"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Simulation" -Key "WaveSpeed" -Value "5.5"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Simulation" -Key "Damping" -Value "0.90"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Simulation" -Key "WindWaveAmplitude" -Value "0.00"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Simulation" -Key "WindWaveLength" -Value "38"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Simulation" -Key "MaxSimulatedDisplacement" -Value "0.00"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Simulation" -Key "ObjectDisturbanceScale" -Value "0.30"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Simulation" -Key "CharacterDisturbanceScale" -Value "0.42"
...
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "ShipPhysics" -Key "ShipWakeStrength" -Value "0.28"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "ShipPhysics" -Key "ShipUprightAssist" -Value "true"
...
Write-Host ""
Write-Host "PhysicalWater 0.3.14 milestone: D3D11 shader semantic repair, explicit D3D11 compilation, unsupported-shader build rejection, and versioned patch workflow."
