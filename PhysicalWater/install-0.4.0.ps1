...
$VersionedChangelogPath = Join-Path $ProjectRoot "CHANGELOG-0.4.0.md"
...
if (Test-Path -LiteralPath $VersionedChangelogPath -PathType Leaf) {
    ...
    Write-Host "Applied CHANGELOG 0.4.0 to canonical project changelog." -ForegroundColor DarkGray
}
...
$FixedShaderPath = Join-Path $UnityPackage "Runtime\Shaders\PhysicalOceanSurface-0.4.0.shader.fixed"
...
if (Test-Path -LiteralPath $FixedShaderPath -PathType Leaf) {
    ...
    Write-Host "Applied PhysicalOceanSurface 0.4.0 concentric LOD spectral ocean shader to canonical G: source." -ForegroundColor DarkGray
}
...
$FixedBundleBuilderPath = Join-Path $ProjectRoot "BuildPhysicalWaterBundle-0.4.0.cs.fixed"
...
& (Join-Path $ProjectRoot "build-0.4.0.ps1") -ValheimDir $ValheimDir -Configuration $Configuration
...
Write-Host ""
Write-Host "PhysicalWater 0.4.0 milestone: Ocean Core Rewrite - concentric camera LOD ocean, deep-water dispersion spectrum shared by CPU/GPU, local interaction field, hydrostatic 3x3 ship buoyancy, no vanilla water."
