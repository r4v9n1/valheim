...
$VersionedChangelogPath = Join-Path $ProjectRoot "CHANGELOG-0.3.18.md"
...
if (Test-Path -LiteralPath $VersionedChangelogPath -PathType Leaf) {
    ...
    Write-Host "Applied CHANGELOG 0.3.18 to canonical project changelog." -ForegroundColor DarkGray
}
...
$FixedShaderPath = Join-Path $UnityPackage "Runtime\Shaders\PhysicalOceanSurface-0.3.18.shader.fixed"
...
if (Test-Path -LiteralPath $FixedShaderPath -PathType Leaf) {
    ...
    Write-Host "Applied PhysicalOceanSurface 0.3.18 authoritative GPU physical-wave + live-ripple shader to canonical G: source." -ForegroundColor DarkGray
}
...
$FixedBundleBuilderPath = Join-Path $ProjectRoot "BuildPhysicalWaterBundle-0.3.18.cs.fixed"
...
& (Join-Path $ProjectRoot "build-0.3.18.ps1") -ValheimDir $ValheimDir -Configuration $Configuration
...
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Visuals" -Key "ShorelineEffectsInterval" -Value "0.16"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Visuals" -Key "ShorelineEffectsBudget" -Value "220"
...
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Simulation" -Key "WindWaveAmplitude" -Value "0.78"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Simulation" -Key "WindWaveLength" -Value "38"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Simulation" -Key "MaxSimulatedDisplacement" -Value "0.24"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Simulation" -Key "ObjectDisturbanceScale" -Value "0.18"
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "Simulation" -Key "CharacterDisturbanceScale" -Value "0.22"
...
Set-PhysicalWaterConfigValue -Path $ConfigPath -Section "ShipPhysics" -Key "ShipWakeStrength" -Value "0.035"
...
Write-Host ""
Write-Host "PhysicalWater 0.3.18 milestone: hard no-vanilla cutover, exclusive PhysicalWater buoyancy/propulsion, low-pass character waterline, GPU physical waves, live ripple texture, own audio/effects, and emergency self-owned visibility material."
